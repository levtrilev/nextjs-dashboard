...
import { createLocation, updateLocation } from "../../lib/locations-actions";
import PdfDocument from "./location-pdf-document";
...
//#region zod schema
const LocationFormSchemaFull = z.object({
  id: z.string().uuid(),
  name: z.string().min(2, {
...
const LocationFormSchema = LocationFormSchemaFull.omit({ id: true, timestamptz: true, username: true, editing_by_user_id: true, editing_since: true });
export type FormData = z.infer<typeof LocationFormSchemaFull>;
//#endregion

export default function LocationEditForm(props: IEditFormProps) {
  ...
  const [formData, setFormData] = useState<FormData>(props.location as FormData);
  ...
  const validate = () => {
    const res = LocationFormSchema.safeParse({
      ...formData,
    });
    if (res.success) {
      return undefined;
