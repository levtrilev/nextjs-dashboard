...
import { updateGood } from './goods-actions';
import Link from 'next/link';
import { Good } from '@/app/lib/definitions';

export function CreateGood({ readonly }: { readonly: boolean }) {
  return (
    <Link
      href={!readonly ? "/erp/goods/create" : "#"}
    ...
    >
      <span className="hidden md:block">Создать товар</span>
      <PlusIcon className="h-5 md:ml-4" />
    </Link>
  );
}

export function BtnUpdateGood({ good }: { good: Good }) {
  const updateGoodWithData = updateGood.bind(null, good);
  return (
    <form action={updateGoodWithData} className="w-full">
      <button
        type="submit"
...
export function BtnEditGoodLink({ id }: { id: string }) {
  const LinkIcon = PencilIcon;
  return (
    <Link
      key={"Edit"}
      href={"/erp/goods/" + id + "/edit"}
