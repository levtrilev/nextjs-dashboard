...
export function CreateCar({ readonly }: { readonly: boolean; }) {
  ...
  return (
    ...
    >
      <span className="hidden md:block">Создать автомобиль</span>
      <PlusIcon className="h-5 md:ml-4" />
    </Link>
  );
}

export function BtnUpdateCar({ car }: { car: Car }) {
  const updateCarWithData = updateCar.bind(null, car);
  return (
    <form action={updateCarWithData} className="w-full">
      <button
        type="submit"
...
export function BtnEditCarLink({ id }: { id: string }) {
