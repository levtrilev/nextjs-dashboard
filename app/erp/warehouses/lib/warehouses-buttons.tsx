...
import { updateWarehouse } from './warehouses-actions';
import Link from 'next/link';
import { Warehouse } from '@/app/lib/definitions';

export function CreateWarehouse({ readonly }: { readonly: boolean }) {
  return (
    <Link
      href={!readonly ? "/erp/warehouses/create" : "#"}
    ...
    >
      <span className="hidden md:block">Создать склад</span>
      <PlusIcon className="h-5 md:ml-4" />
    </Link>
  );
}

export function BtnUpdateWarehouse({ warehouse }: { warehouse: Warehouse }) {
  const updateWarehouseWithData = updateWarehouse.bind(null, warehouse);
  return (
    <form action={updateWarehouseWithData} className="w-full">
      <button
        type="submit"
...
export function BtnEditWarehouseLink({ id }: { id: string }) {
  const LinkIcon = PencilIcon;
  return (
    <Link
      key={"Edit"}
      href={"/erp/warehouses/" + id + "/edit"}
