...
import { updateWork } from './works-actions';
import Link from 'next/link';
import { Work } from '@/app/lib/definitions';

export function CreateWork({ readonly }: { readonly: boolean; }) {

  return (
    <Link
      href={!readonly ? "/repair/works/create" : "#"}
    ...
    >
      <span className="hidden md:block">Создать Работу</span>
      <PlusIcon className="h-5 md:ml-4" />
    </Link>
  );
}

export function BtnUpdateWork({ work }: { work: Work }) {
  const updateWorkWithData = updateWork.bind(null, work);
  return (
    <form action={updateWorkWithData} className="w-full">
      <button
        type="submit"
...
export function BtnEditWorkLink({ id }: { id: string }) {
  const LinkIcon = PencilIcon;
  return (
    <Link
      key={"Edit"}
      href={"/repair/works/" + id + "/edit"}
