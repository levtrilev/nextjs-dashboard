...
import { fetchPersonForm, tryLockRecord, unlockRecord } from "../../lib/persons-actions";
...
async function Page(props: { params: Promise<{ id: string }> }) {
    ...
    let canEdit = false;
    if (isEditable) {
        const lockResult = await tryLockRecord(person.id, user.id);
    ...
    const freshRecordRes = await pool.query(
        `SELECT persons.editing_by_user_id, users.email as editing_by_user_email
        FROM persons 
        LEFT JOIN users on persons.editing_by_user_id = users.id 
        WHERE persons.id = $1`
        , [person.id]);
