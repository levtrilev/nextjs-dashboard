...
import { updatePerson } from './persons-actions';
import Link from 'next/link';
import { Person } from '@/app/lib/definitions';

export function CreatePerson({ readonly }: { readonly: boolean; }) {

  return (
    <Link
      href={!readonly ? "/repair/persons/create" : "#"}
    ...
    >
      <span className="hidden md:block">Создать Сотрудника</span>
      <PlusIcon className="h-5 md:ml-4" />
    </Link>
  );
}

export function BtnUpdatePerson({ person }: { person: Person }) {
  const updatePersonWithData = updatePerson.bind(null, person);
  return (
    <form action={updatePersonWithData} className="w-full">
      <button
        type="submit"
...
export function BtnEditPersonLink({ id }: { id: string }) {
  const LinkIcon = PencilIcon;
  return (
    <Link
      key={"Edit"}
      href={"/repair/persons/" + id + "/edit"}
