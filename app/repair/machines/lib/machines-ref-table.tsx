import { Machine, MachineForm } from "@/app/lib/definitions";
import RefSearch from "@/app/ui/ref-search";

interface IMachinesRefTableProps {
...
export default function MachinesRefTable(props: IMachinesRefTableProps) {
                            ...
                            <table className="table-fixed hidden w-full rounded-md text-gray-900 md:table">
                                <tbody className="divide-y divide-gray-200 text-gray-900">
                                    {props.machines.map((machine) => (((
                                        machine.name.toLowerCase().includes(props.term.toLowerCase())
                                        || props.term.length === 0) &&
                                                ...
                                                <div className="flex items-left gap-3">
                                                    <a
                                                        onClick={(e) => {
                                                            e.preventDefault();
                                                            e.stopPropagation();
                                                            props.handleSelectMachine(machine.id, machine.name, machine.location_id, machine.location_name);
                                                            props.setTerm("");
                                                            props.closeModal();
                                                        }}
                                                        // href={"#"}
                                                        className="text-blue-800 underline cursor-pointer hover:text-blue-600"
                                                    >{machine.name.substring(0, 36)}</a>
                                                </div>
                                            </td>
                                ...
                                >
                                    <div className="font-medium text-black">
                                        <a
                                            ...
                                            className="text-blue-800 underline cursor-pointer hover:text-blue-600"
                                        >
                                            {machine.name.substring(0, 36)}
                                        </a>
                                    </div>
                                    <div className="text-gray-500">{machine.location_name}</div>
                                    <div className="text-gray-500">{machine.machine_status}</div>
                                </div>
                            ))}
                    </div>
                </div>
