...
import { createPart, updatePart } from "../../lib/parts-actions";
import PdfDocument from "./part-pdf-document";
...
//#region zod schema
const PartFormSchemaFull = z.object({
  id: z.string().uuid(),
  name: z.string().min(2, {
...
const PartFormSchema = PartFormSchemaFull.omit({ id: true, timestamptz: true, username: true, editing_by_user_id: true, editing_since: true });
export type FormData = z.infer<typeof PartFormSchemaFull>;
//#endregion

export default function PartEditForm(props: IEditFormProps) {
  ...
  const [formData, setFormData] = useState<FormData>(props.part as FormData);
  ...
  const validate = () => {
    const res = PartFormSchema.safeParse({
      ...formData,
  ...
  const handleBackClick = async (e: React.MouseEvent) => {
    e.preventDefault();
    if (props.unlockAction) await props.unlockAction("parts", props.part.id, sessionUserId);
