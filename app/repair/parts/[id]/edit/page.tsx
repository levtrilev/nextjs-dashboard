...
import { fetchPartForm, tryLockRecord, unlockRecord } from "../../lib/parts-actions";
...
async function Page(props: { params: Promise<{ id: string }> }) {
    ...
    const part: PartForm = await fetchPartForm(id);
    if (!part) {
    ...
    let canEdit = false;
    if (isEditable) {
        const lockResult = await tryLockRecord(part.id, user.id);
    ...
    const freshRecordRes = await pool.query(
        `SELECT parts.editing_by_user_id, users.email as editing_by_user_email
        FROM parts 
        LEFT JOIN users on parts.editing_by_user_id = users.id 
        WHERE parts.id = $1`
        , [part.id]);
