// Delete System button.tsx

'use client';
import { TrashIcon } from '@heroicons/react/24/outline';
import { useState, useRef } from 'react';
import MessageBoxSrv from '@/app/lib/message-box-srv';
import { deleteSystem } from './systems-actions';

export default function BtnDeleteSystem({ id, name }: { id: string, name: string }) {
  ...
  const askUserForDeleting = (id: string, name: string) => {
    ...
    setMessageBoxText(`Система(признак): ${name} \nУдалить систему(признак)?`);
  };

  const deleteTaskWithId = askUserForDeleting.bind(null, id, name);

  const handleOK = () => {
    deleteSystem(idToDelete.current);
