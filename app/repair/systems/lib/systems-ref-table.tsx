...
interface ISystemsRefTableProps {
    systems: System[],
...
export default function SystemsRefTable(props: ISystemsRefTableProps) {
    ...
    return (
        <div className="w-full">
            ...
            <RefSearch callback={handleSearch} term={props.term} elementIdPrefix="system" />
                            ...
                            <table className="table-fixed hidden w-full rounded-md text-gray-900 md:table">
                                <tbody className="divide-y divide-gray-200 text-gray-900">
                                    {props.systems.map((system) => (((
                                        system.name.toLowerCase().includes(props.term.toLowerCase())
                                        || props.term.length === 0) &&
                                                ...
                                                <div className="flex items-left gap-3">
                                                    <a
                                                        onClick={(e) => {
                                                            e.preventDefault();
                                                            e.stopPropagation();
                                                            props.handleSelectSystem(system.id, system.name);
                                                            props.setTerm("");
                                                            props.closeModal();
                                                        }}
                                                        className="text-blue-800 underline cursor-pointer hover:text-blue-600"
                                                    >{system.name.substring(0, 36)}</a>
                                                </div>
                                            </td>
