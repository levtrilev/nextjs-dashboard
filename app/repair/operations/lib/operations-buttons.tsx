...
import { updateOperation } from './operations-actions';
import Link from 'next/link';
import { Operation } from '@/app/lib/definitions';

export function CreateOperation({ readonly }: { readonly: boolean }) {
  return (
    <Link
      href={!readonly ? "/repair/operations/create" : "#"}
    ...
    >
      <span className="hidden md:block">Создать Операцию</span>
      <PlusIcon className="h-5 md:ml-4" />
    </Link>
  );
}

export function BtnUpdateOperation({ operation }: { operation: Operation }) {
  const updateOperationWithData = updateOperation.bind(null, operation);
  return (
    <form action={updateOperationWithData} className="w-full">
      <button
        type="submit"
...
export function BtnEditOperationLink({ id }: { id: string }) {
  const LinkIcon = PencilIcon;
  return (
    <Link
      key={"Edit"}
      href={"/repair/operations/" + id + "/edit"}
