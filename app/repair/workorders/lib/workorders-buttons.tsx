...
import { updateWorkorder } from './workorders-actions';
import Link from 'next/link';
import { Workorder } from '@/app/lib/definitions';

export function CreateWorkorder({ readonly }: { readonly: boolean }) {
  return (
    <Link
      href={!readonly ? "/repair/workorders/create" : "#"}
    ...
    >
      <span className="hidden md:block">Создать Наряд</span>
      <PlusIcon className="h-5 md:ml-4" />
    </Link>
  );
}

export function BtnUpdateWorkorder({ workorder }: { workorder: Workorder }) {
  const updateWorkorderWithData = updateWorkorder.bind(null, workorder);
  return (
    <form action={updateWorkorderWithData} className="w-full">
      <button
        type="submit"
...
export function BtnEditWorkorderLink({ id }: { id: string }) {
  const LinkIcon = PencilIcon;
  return (
    <Link
      key={"Edit"}
      href={"/repair/workorders/" + id + "/edit"}
